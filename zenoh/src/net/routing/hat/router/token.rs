...
fn propagate_forget_simple_token(
    ...
    // FIXME(fuzzypixelz): This function should be split to accomodate for the divergent code
    // paths (the ones coming from `HatTokenTrait::undeclare_token` vs the ones coming from
    // `HatBaseTrait::closing` and `HatBaseTrait::handle_oam`).
